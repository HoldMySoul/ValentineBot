...
from ValentineBot import DB_NAME, OWNER_ID, dispatcher, LOGGER as log, BACKUP_PASS
from ValentineBot.modules.helper_funcs.chat_status import owner_plus
...
def backup_db(_: CallbackContext):
    ...
    if not os.path.exists('{}/{}'.format(bkplocation, dbbkpname)):
        ...
        if os.path.exists('ValentineBot/config.py'):
            print("config copied")
            shutil.copyfile('ValentineBot/config.py', '{}/config.py'.format(bkplocation))
...
from ValentineBot import updater as u
