from ValentineBot.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
from ValentineBot import SUDO_USERS
...
async def saitama_is_admin(chat_id: int):
    status = False
    ValentineBot = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if ValentineBot.id == user.id:
            status = True
            break
    return status
